  ...
  switch (ST_allarme) {
    ...
    case ST_ALLARME_ALLERT_BUZZER1://Allarme non attivabile
      ...
      if (allarme_counter0 >= ALLARME_CICLI_BEEP(3)) {
        ...
        send_email_message(notify_message_allarme);
    ...
    case ST_ALLARME_ALLERT_BUZZER3:
      ...
      if (CHECKBITMASK(mask_esclusioni, BITMASK8(BIT_TAMPER))) { //Beep per indicare che il tamper è escluso
        ...
        send_telegram_message("ALLARME INSERITO CON SUCCESSO");
        send_email_message("Allarme Inserito con successo");
        STOP_TIMER(allarme_timer0);
        ST_allarme = ST_ALLARME_BUILD_TEMP_MASK; //Inizia il ciclo di allarme
      }
      break;
    ...
    case ST_ALLARME_CYCLE_SIRENA_ON:
      ...
      if (!ISALIVE_TIMER(allarme_timer1)) { // Se la sirena è spenta ma non è partito il timer allora fa partire la sirena
        ...
        if (GETBIT8(hold_allarm_sensor & (~(mask_temp_esclusioni | mask_esclusioni)), BIT_ZONE1)) notify_message_allarme += "Zona1\t";
        if (GETBIT8(hold_allarm_sensor & (~(mask_temp_esclusioni | mask_esclusioni)), BIT_ZONE2)) notify_message_allarme += "Zona2\t";
        if (GETBIT8(hold_allarm_sensor & (~(mask_temp_esclusioni | mask_esclusioni)), BIT_ZONE3)) notify_message_allarme += "Zona3\t";
        if (GETBIT8(hold_allarm_sensor & (~(mask_temp_esclusioni | mask_esclusioni)), BIT_ZONE4)) notify_message_allarme += "Zona4\t";
        if (GETBIT8(hold_allarm_sensor & (~(mask_temp_esclusioni | mask_esclusioni)), BIT_TAMPER)) notify_message_allarme += "\n!!Manomissione rilevata!!\t";
    ...
    case ST_ALLARME_TAMPER_MANOMESSO:
      ...
      if (!ISALIVE_TIMER(allarme_timer1)) { // Se la sirena è spenta ma è partito il timer allora fa partire la sirena
        ...
        new_st_sirena = SIRENA_ON;
        //notifico che c'è stato un'evento d'allarme
        notify_message_allarme = "!! MANOMISSIONE RILEVATA !!\n";
        send_telegram_message(notify_message_allarme);
        send_email_message(notify_message_allarme);
        START_TIMER(allarme_timer1); // fa partire il timer per il tempo di ciclo sirena
    ...
    case ST_ALLARME_RESET_ALL:
      ...
      BITMASKRESET(hold_allarm_sensor);
      send_telegram_message("Allarme DISATTIVATO con successo");
      send_email_message("Allarme DISATTIVATO con successo");
