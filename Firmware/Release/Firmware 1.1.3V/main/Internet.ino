void InternetLoop() {
  ...
  switch (internet_st) {
    ...
    case INTERNET_CONNECTION1://Configura l'interfaccia di rete
      ...
      Ethernet.begin(mac, 10 * SECONDS, 4 * SECONDS); //Imposto i timeout della periferica ethernet
#endif
    ...
    case INTERNET_CONNECTION3: // rimane in attesa di connessione
      ...
      switch (Ethernet.linkStatus()) {
        case LinkON:
          if ((Ethernet.localIP()[0] == 0) && (Ethernet.localIP()[1] == 0) && (Ethernet.localIP()[2] == 0) && (Ethernet.localIP()[3] == 0)) { //Se è la prima volta che collega il cavo Internet torna allo stato connection1
            internet_st = INTERNET_CONNECTION1; //Ritenta l'inizializzazzione della scheda 
          } else {
    ...
    case INTERNET_SET_NTP:
      ...
      while (getLocalTime(&RTCTime) && (!IS_DELAYED(internet_timer1, 15 * SECONDS))) { // Sincronizza l'orologgio per la prima volta - va in timeout se entro 15s non riesce a sincronizzarsi con il server
        ...
        TASKDELAY(100); //Attende 100ms tra una richiesta e l'altra
      }
